...
def training(model, x, y):
    ...
    for _ in range(n_steps):
        ...
        indices = torch.randint(0, x.shape[0], size=(minibatch_size,))
        x_batch = x[indices]
        y_batch = y[indices]
        ...
        # backward pass
        model.zero_grad()  # zerowanie gradientów
        loss.backward()  # autograd - policzenie gradientów metodą wstecznej propagacji
